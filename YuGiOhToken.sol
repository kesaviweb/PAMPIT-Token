/**
   YuGiOhToken Features

    199,900,000,000 Total Supply

    ❗️FOR EACH TRANSACTION:

    10% will be a dividend for the holders
    5% added back to liquidity
...
contract YuGiOhToken is Context, IERC20, Ownable {
    ...
    string private _name = "YuGiOhToken";
    string private _symbol = "YGOT";
    ...
    address public marketingWallet = 0x0bAbE9DFDD0ca74756A1ad99AA7f44235e978de7;
    ...
    function _getTValues(uint256 tAmount) private returns (uint256, uint256) {
        ...
        uint256 tOtherFees = tLiquidity.add(tMarketing).add(tBurn);
        uint256 tTransferAmount = tAmount.sub(tOtherFees);
        return (tTransferAmount, tOtherFees);
    }

    function _getRValues(uint256 tAmount, uint256 tOtherFees, uint256 currentRate) private pure returns (uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rOtherFees = tOtherFees.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rOtherFees);
    ...
    function _calculateMarketingFee (uint256 tAmount) private returns (uint256) {
    ...
    function _calculateBurnFee (uint256 tAmount) private returns (uint256) {
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, other fees
